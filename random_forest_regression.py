...
X = dataset.iloc[:, 1:numberOfFeatures].values
...
randomforest = RandomForestRegressor(n_estimators = 21, random_state = 0)
...
# performance metrics
rSquared = r2_score(regressionRes, y_test)
adjustedRSquared = 1 - (1-rSquared)*(len(y)-1)/(len(y)-X.shape[1]-1)
print(rSquared)
